    ...
    <div class="layui-row">
        <form class="layui-form">
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <input type="text" id="u_img" name="u_img" required="" lay-verify="u_img" autocomplete="off"
...
<script>
    layui.use(['form','layer'], function() {
        ...
        form.on('submit(add)', function (data) {
            $.ajax({
                url: './user',
                type: 'post',
                data: {'method':'editAdmin', 'id':data.field.id, 'username':data.field.username, 'pass':data.field.pass, 'repass':data.field.repass, 'email':data.field.email, 'tel':data.field.tel, 'u_img':data.field.u_img},
                dataType: 'json',
    ...
    // 监听页面元素加载结束后的动作
    $(function() {
        $("#L_email").val(getQueryVariable("u_email"));
        $("#L_username").val(decodeURI(getQueryVariable("u_name")));
        $("#id").val(getQueryVariable("id"));
        $("#L_tel").val(getQueryVariable("u_tel"));
        $("#u_img").val(getQueryVariable("u_img"));
    });
</script>
</html>
